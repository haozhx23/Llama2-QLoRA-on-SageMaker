...
def generate_prompt(tokenizer, prompt, completion=None):
    sql_comp_template = "{prompt}\n\n{completion}"+tokenizer.eos_token
    
    if None == completion:
        return prompt
    
    res = sql_comp_template.format(prompt=prompt,completion=completion)

    return res

def tokenize(tokenizer, prompt, max_length, add_eos_token=False):
    result = tokenizer(
        prompt,
        truncation=True,
...
def generate_and_tokenize_prompt(data_point, max_length, tokenizer):
    ...
    user_prompt = generate_prompt(tokenizer, data_point["prompt"])
    tokenized_user_prompt = tokenize(tokenizer, user_prompt, max_length)
...
def preprocess_dataset(tokenizer: AutoTokenizer, max_length: int, seed, dataset: str):
    _preprocessing_function = partial(generate_and_tokenize_prompt, max_length=max_length, tokenizer=tokenizer)
...
def train(model, tokenizer, dataset, output_dir):
    ...
    trainer = Trainer(
        ...
        args=TrainingArguments(
            ...
            optim="paged_adamw_8bit", #paged_adamw_32bitEW
        ),
